...
int dc_target_get_instance()
{
	int result;
	
	// Get instance value.
	result = getlocalvar(DC_TARGET_MEMBER_INSTANCE);

	// Value must be a valid integer, or
	// we will use the default.
	if (typeof(result) != openborconstant("VT_INTEGER"))
	{
		result = DC_TARGET_DEFAULT_INSTANCE;
	}
...
// 2018-11-29
// Set instance value. 
void dc_target_set_instance(int value)
{
	// Only set instance value if new value
	// is different from default. No reason to waste
	// the memory on a default value.
	if (value != DC_TARGET_DEFAULT_INSTANCE)
	{
		setlocalvar(DC_TARGET_MEMBER_INSTANCE, value);
	}	
}
...
void dc_target_free_instance()
{
	...
	for (i = 0; i < DC_TARGET_MEMBER_THE_END; i++)
	{
...
// to import.
void dc_target_export_instance()
{
	int instance;
	int i;
	char id;
...
void dc_target_import_instance()
{
	int instance;
	int i;
	char id;
...
void dc_target_free_export_instance()
{
	int instance;
	int i;
	char id;
